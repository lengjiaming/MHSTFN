...
def read_and_generate_dataset(graph_signal_matrix_filename,graph_signal_matrix_filename2,
    ...
    max_count = np.maximum(max_count1, max_count2)  # 逐元素最大值，shape: (1, 817, 1)
    max_tensor = np.tile(max_count, (26208, 1, 1))
    merged_seq = np.concatenate([data_seq, max_tensor], axis=-1)
    ...
    if save:
        ...
        filename = os.path.join(dirpath, file +"-mask-dataloader")
...
all_data = read_and_generate_dataset(graph_signal_matrix_filename,graph_signal_matrix_filename2, 0, 0, 1, num_for_predict, points_per_hour=points_per_hour, save=True)
