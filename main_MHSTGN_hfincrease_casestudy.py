...
import argparse
import configparser
from model.MHSTGN_hefei_increase import make_model
from lib.utils_MHSTGN_hefei_increase_casestudy import *
...
his_mask_adj_filename =  data_config['his_mask_adj_filename']
trans_mask_adj_filename =  data_config['trans_mask_adj_filename']

graph_signal_matrix_filename = "./data_aaai25/hf-charge-increase-casestudyrandom.npz"
...
lambda_linke = float(training_config['lambda_linke'])
...
his_adj_mx = torch.tensor(his_adj_mx, dtype=torch.float32).to(DEVICE) 
trans_adj_mx  = torch.tensor(trans_adj_mx , dtype=torch.float32).to(DEVICE)
his_adj_mx_mask = get_adjacency_matrix(his_mask_adj_filename, num_of_vertices)
trans_adj_mx_mask = get_adjacency_matrix(trans_mask_adj_filename, num_of_vertices)
# edge_features = np.stack([dis_adj_mx, poi_adj_mx], axis=0)
edge_features = np.stack([dis_adj_mx,poi_adj_mx,his_adj_mx_mask,trans_adj_mx_mask], axis=0)
...
net = make_model(DEVICE, seed,nb_block, in_channels, node_dim, edge_dim, num_of_graph,  num_of_vertices,edge_features,adjacency_matrix,num_prototypes,len_input,num_for_predict,num_of_expert,top_k)

# # 将模型发送到默认设备
# net.to(DEVICE)
def register_memory_hooks(model: nn.Module):
    memory_info = {}
...
val_loader, val_target_tensor, test_loader, test_target_tensor = load_graphdata_channel2(
    graph_signal_matrix_filename, DEVICE, batch_size)
mean = _mean.squeeze()  # 变成 [1,1,1]
std = _std.squeeze()
mean = torch.tensor(mean, dtype=float, device=DEVICE)
std = torch.tensor(std, dtype=float, device=DEVICE)
...
def train_main(mean,std,his_adj_mx,trans_adj_mx):
    ...
    # train model
    his_adj_mx= his_adj_mx.unsqueeze(0).repeat(batch_size, 1, 1)
    trans_adj_mx= trans_adj_mx.unsqueeze(0).repeat(batch_size, 1, 1)
    for epoch in range(start_epoch, epochs):
        current_time = datetime.datetime.now()
        print(f"Epoch {epoch+1}/{epochs} at {current_time}")
        params_filename = os.path.join(params_path, f'epoch_{epoch}.params')
        
        val_loss = compute_val_loss_mstgcn(net, val_loader,lambda_contrastive,edge_features, criterion, masked_flag, sw, epoch,mean,std,his_adj_mx,trans_adj_mx,lambda_linke,batch_size)
        evaluate_on_test_mstgcn(net, test_loader, test_target_tensor,edge_features, epoch,mean,std)
        ...
        for batch_index, batch_data in tqdm(enumerate(train_loader), total=len(train_loader), desc="Batch Progress", leave=False):
            ...
            outputs, contrastive_loss, edgepre_his,edgepre_pref = net(encoder_inputs, timestamps)
            
            outputs= outputs*std+mean
            outputs = outputs.squeeze(2)
            if(encoder_inputs.shape[0]!=batch_size):
                his_adj_mx= his_adj_mx[0].repeat(encoder_inputs.shape[0], 1, 1)
                trans_adj_mx= trans_adj_mx[0].repeat(encoder_inputs.shape[0], 1, 1)
                edgepre_his_loss = label_loss_fn(edgepre_his, his_adj_mx)
                edgepre_pref_his_loss = label_loss_fn(edgepre_pref, trans_adj_mx)
                his_adj_mx= his_adj_mx[0].repeat(batch_size, 1, 1)
                trans_adj_mx= trans_adj_mx[0].repeat(batch_size, 1, 1)
            else:    
                edgepre_his_loss = label_loss_fn(edgepre_his, his_adj_mx)
                edgepre_pref_his_loss = label_loss_fn(edgepre_pref, trans_adj_mx)
            prediction_loss = criterion(outputs, labels)
            loss = prediction_loss + lambda_contrastive * contrastive_loss + lambda_linke * edgepre_his_loss + lambda_linke * edgepre_pref_his_loss 
            # print(f'linkloss: {edgepre_pref_his_loss}')
            loss.backward()
            # optimizer.step()
        ...
        train_loss_str = (
            ...
            epoch_contrastive_loss / batch_count,
            epoch_loss / batch_count
        )
        with open('result_hefei_increase/train_losses_count_link16_lr2-4_prot256_k4_case.txt', 'a', encoding='utf-8') as file:
...
if __name__ == "__main__":
    ...
    # train_main(mean,std,his_adj_mx,trans_adj_mx)
    # mask_nodes =[682, 642, 782, 580, 767, 661, 228, 409, 632, 804, 210, 590, 596,
    #    814, 189, 701, 676, 382, 720, 462,  91, 707, 626, 369, 633, 176,
    #    737, 746, 433, 466, 574, 730, 389,  94, 122, 365, 256, 204, 359,
    #    754, 485, 394,  11,  17, 249, 605, 395, 268, 599, 560, 105, 336,
    #     24, 351, 798,  82, 542, 290, 401, 370, 733, 606, 140, 702,  67,
    #    476, 427, 537, 494, 111, 319, 423, 535, 302, 589, 406, 518,  47,
    #    405,  34, 236, 584, 712, 615,  57, 282, 425, 385, 310, 128, 250,
    #    371, 119, 578, 470,  35, 380, 221, 127,  88, 781, 506, 196, 349,
    #    291, 651, 284, 384, 604, 334,  36, 440,  71, 628, 159, 683, 439,
    #    230, 658, 808, 773, 293, 743, 771, 109, 429, 147, 514,  68, 695,
    #    286, 143,  38, 558, 144, 390, 214, 163, 237, 625, 190, 807, 567,
    #    696,  19, 638, 342, 460, 553, 323,   9, 643, 577, 468, 421, 416,
    #    750, 631, 220, 613, 566, 519, 317]
    predict_main(79,test_loader, test_target_tensor, mean, std, 'test')
