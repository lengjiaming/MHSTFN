...
def read_and_generate_dataset(graph_signal_matrix_filename,
    ...
    max_count = np.max(data_seq, axis=0, keepdims=True)
    # max_tensor = np.tile(max_count, (26208, 1, 1))
    max_tensor = np.tile(max_count, (8640, 1, 1))
    merged_seq = np.concatenate([data_seq, max_tensor], axis=-1)

    all_samples = []
    for idx in range(data_seq.shape[0]):
        sample = get_sample_indices(merged_seq, num_of_weeks, num_of_days,
    ...
    if save:
        ...
        filename = os.path.join(dirpath, file +"-dataloader")
...
all_data = read_and_generate_dataset(graph_signal_matrix_filename, 0, 0, 1, num_for_predict, points_per_hour=points_per_hour, save=True)
