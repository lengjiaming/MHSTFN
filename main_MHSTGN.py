...
import argparse
import configparser
from model.MHSTGN import make_model
from lib.utils_MHSTGN import *
...
edges_dis = dis_adj_mx > 0
edges_poi = poi_adj_mx > 0
edges_his = his_adj_mx > 0
edges_sum = edges_dis + edges_poi + edges_his
# 统计至少有两种边的节点对
true_count = np.sum(edges_sum)

# 计算所有节点对的总数
total_pairs = np.sum(edges_dis)+np.sum(edges_poi)+np.sum(edges_his)   # 无向图，去掉对称对

# 计算比例
proportion = true_count / total_pairs

print(f"Proportion of node pairs with at least two edges: {proportion:.4f}")

# trans_adj_mx = get_adjacency_matrix(trans_adj_filename, num_of_vertices)
edge_features = np.stack([dis_adj_mx, poi_adj_mx, his_adj_mx], axis=0)
...
def train_main():
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_index, batch_data in tqdm(enumerate(train_loader), total=len(train_loader), desc="Batch Progress", leave=False):
            ...
            outputs, contrastive_loss = net(encoder_inputs, timestamps)
            
            outputs = outputs.squeeze(2)
            prediction_loss = criterion(outputs, labels)
            loss = prediction_loss + lambda_contrastive * contrastive_loss
        ...
        with open('result_shenzhen_final/train_losses_all_lambda0.1.txt', 'a', encoding='utf-8') as file:
...
if __name__ == "__main__":
    ...
    # predict_main(137, test_loader, test_target_tensor, _mean, _std, 'test')
