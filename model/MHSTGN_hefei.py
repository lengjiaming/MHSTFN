...
def time_encoding(minutes_tensor):
    ...
    hours = expanded_tensor  % 288
    weekdays = (expanded_tensor // (288)) % 7
    hours = hours.int()
...
class NodeEmbeddingNetwork(nn.Module):
    def __init__(self, DEVICE, in_channels, conv_dim, embed_dim):
        ...
        self.conv1 = nn.Conv2d(1, conv_dim // 2, kernel_size=(1, 3), padding=(0, 1))  # 第一层卷积
...
class SupernodeAssignmentLayer(nn.Module):
    def __init__(self, DEVICE, feature_dim, dropout,  latend_num):
        """
        初始化分配层
        :param feature_dim: 节点特征维度 F
        :param num_supernodes: 超节点数 K
        """
        super(SupernodeAssignmentLayer, self).__init__()
        # 定义可学习的分配矩阵 W，其形状为 (F, K)
        self.dropout = nn.Dropout(dropout)
        self.w_classify = nn.Linear(feature_dim, latend_num, bias=True)
        self.DEVICE = DEVICE

        self.to(DEVICE)

    def apply_bn(self, x):
        # Batch normalization of 4D tensor x with shape (B, T, K, F)
        B, T, K, F = x.size()
        
        # 将输入张量重塑为 (B * T, K, F) 这样我们可以在 K 上进行批量归一化
        x = x.view(B * T, K, F)
        
        # 使用 BatchNorm1d 对 K 维度进行归一化
        bn_module = nn.BatchNorm1d(K).cuda()  # BatchNorm1d 是针对 K 维度进行归一化
        x = bn_module(x)  # 对 (B * T, K, F) 进行归一化
    ...
    def forward(self, x):
        """
        前向传播
        :param x: 输入的节点表示, 形状为 (B, N, T, F)
        :return: K个超节点的表示, 形状为 (B, K, T, F)
        """
        B, N, T, in_features = x.size()
        h_now = self.dropout(x) # (B, N, T, F)
        S = self.w_classify(h_now) # (B, N, T,latend_num(K))
        S = F.softmax(S,dim=-1) # (B, N, T, K)
        S = S.permute(0, 2, 3, 1)  # (B, T, K, N)
        h_now = h_now.permute(0, 2, 1, 3)  # (B, T, N, F)
        
        h_c = torch.einsum('btkn,btnf->btkf', S, h_now)  # (B, T, K, F)
        h_c = self.apply_bn(h_c)  # 对每个时间步进行批量归一化，(B, T, K, F)
        # return h_c.permute(0, 2, 1, 3)
        S = S.permute(0, 1, 3, 2)  # (B, T, N, K)
        h_sc = torch.einsum('btnk,btkf->btnf', S, h_c)  # (B, T, N, F)
        
        h_sc = h_sc.permute(0, 2, 1, 3)  # (B, N, T, F)
        return h_sc

class SubgraphPooling(nn.Module):
    def __init__(self,DEVICE, num_angles, num_subgraphs,node_labels, dropout):
        super(SubgraphPooling, self).__init__()
        self.num_angles = num_angles
        self.num_subgraphs = num_subgraphs  # 子图数量，即 K
        self.node_labels = node_labels.to(DEVICE)
        # self.node_labels.require_grad = False
        self.dropout = nn.Dropout(dropout)
        self.DEVICE = DEVICE
    ...
    def apply_bn(self, x):
        ...
        return x
    def masked_softmax(self, matrix, dim):
        """
        对矩阵的非零部分应用 softmax，保持零值为零。
        ...
        返回:
        - softmax_matrix: 处理后的矩阵，零值保持为零，非零值按 softmax 分布
        """
        # 创建一个 mask，标记 matrix 中非零的部分
        mask = (matrix != 0).float()

        # 使用一个大的负数来掩蔽零值，使它们不会影响 softmax 计算
        masked_matrix = matrix.masked_fill(mask == 0, float('-inf'))

        # 对掩蔽后的矩阵进行 softmax
        softmax_matrix = F.softmax(masked_matrix, dim=dim)
    ...
    def forward(self, x):
        ...
        输入:
        - x: [B, N, T, F]，原始节点表示
        - node_labels: [N, M]，表示每个角度上节点所属的子图标签

        输出:
        - subgraph_representation: [B, K, T, F]，子图级别的聚合表示
        """
        B, N, T, feature_dim = x.shape
        h_now = self.dropout(x) # (B, N, T, F)
        # node_labels = F.softmax(self.node_labels,dim=-1) #(N, M)
        node_labels = self.masked_softmax(self.node_labels,dim=0)
        repeated_node_labels = node_labels.unsqueeze(0).unsqueeze(2).repeat(B, 1, T, 1) 
        subgraph_representation = torch.einsum('bntm,bntf->bmtf', repeated_node_labels, h_now)

        h_c = subgraph_representation.permute(0,2,1,3)  # (B, T, M, F)
        h_c = self.apply_bn(h_c)  # 对每个时间步进行批量归一化，(B, T, M, F)
        # return h_c.permute(0, 2, 1, 3)
        S = repeated_node_labels.permute(0, 2, 1, 3)  # (B, T, N, M)
        h_sc = torch.einsum('btnk,btkf->btnf', S, h_c)  # (B, T, N, F)
...
class WeightedSubgraphAggregator(nn.Module):
    def __init__(self, DEVICE,adjacency_matrices, num_nodes, num_edgetype,embed_dim,num_prototypes,len_input):
        ...
        self.to(DEVICE)
    def forward(self, node_feat, node_weights):
        """
        ...
        for edge_type in range(self.adjacency_matrices.shape[0]):
            # 取当前边类型的邻接矩阵
            ...
            subgraph_features.append(fused_memory)  # 列表中每个元素是 [B, N, T, F]
...
class MVSTFN(nn.Module):
    def __init__(self, DEVICE, nb_block, in_channels, node_feature_dim, edge_feature_dim, num_of_graph, num_of_global_vertices,edge_features,adjacency_matrix,num_prototypes,len_input,num_for_predict):
        ...
        self.time_encode_hour = TemporalHourEncoding(DEVICE,node_feature_dim, max_len=288)
        ...
        self.subgraph_aggregator = WeightedSubgraphAggregator(DEVICE,edge_features, num_of_global_vertices,num_of_graph,node_feature_dim,num_prototypes,len_input)
        ...
        self.linear1 = nn.Linear(node_feature_dim*(num_of_graph+1), node_feature_dim)
        self.linear2 = nn.Linear(node_feature_dim*(num_of_graph+1), node_feature_dim)
    ...
    def forward(self, data,  timestamps):       
        ...
        # start_time = time.time()
        node_memory,subgraph_memory = self.subgraph_aggregator(nodefeat,edgefeat)
        # fused_output = node_memory
        # print(f"subgraph_aggregator time: {time.time() - start_time:.4f} seconds")
        # start_time = time.time()
        # start_time = time.time()
        contrastive_loss = self.contrastiveLoss(node_memory,subgraph_memory)
        # contrastive_loss = torch.tensor(0.001)
        # print(f"contrastiveLoss time: {time.time() - start_time:.4f} seconds")
        # start_time = time.time()
        hours, weekdays = time_encoding(timestamps)     

        position_encoded_hour = self.time_encode_hour(node_memory, hours)
     
        position_encoded_day = self.time_encode_week(node_memory, weekdays)

        node_memory_hour = node_memory + position_encoded_hour
        node_memory_day = node_memory + position_encoded_day
        subgraph_memory_hour = subgraph_memory + position_encoded_hour.unsqueeze(-1)
        subgraph_memory_day = subgraph_memory + position_encoded_day.unsqueeze(-1)
        subgraph_memory = torch.cat([subgraph_memory_hour,subgraph_memory_day],-1)

        #replace attention
        fused_output_hour =torch.cat([subgraph_memory_hour,node_memory_hour.unsqueeze(-1)],-1)    
        fused_output_day =torch.cat([subgraph_memory_day,node_memory_day.unsqueeze(-1)],-1) 
        fused_output_hour = self.linear1(fused_output_hour.flatten(start_dim=-2))
        fused_output_day = self.linear1(fused_output_day.flatten(start_dim=-2))
        ...
        # fused_output = self.cross_attn_layer_hour(node_memory, subgraph_memory)
        # start_time = time.time()
        ...
        # print(" ")
        return final_output,contrastive_loss
...
def make_model(DEVICE, nb_block, in_channels, node_dim, edge_dim, num_of_graph, num_of_vertices,edge_features,adjacency_matrix,num_prototypes,len_input,num_for_predict):
    ...
    model = MVSTFN(DEVICE, nb_block, in_channels, node_dim, edge_dim, num_of_graph, num_of_vertices,edge_features,adjacency_matrix,num_prototypes,len_input,num_for_predict)
    seed = 2025
