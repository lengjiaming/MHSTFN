...
class SubgraphMOE(nn.Module):
    ...
    def forward(self, maingraph,subgraph):
        ...
        top_k_probs = top_k_probs / top_k_probs.sum(dim=-1, keepdim=True)  # [B, (C+1)N, T, k]
        # Step 4: 获取专家输出
...
class MVSTFN(nn.Module):
    ...
    def forward(self, data,  timestamps):       
        ...
        # start_time = time.time()
        maingraph_feat,subgraph_feat = self.subgraph_aggregator(nodefeat,edgefeat)
        ...
        # B=data.size(0)
        fused_output = torch.tanh(self.t_proj(torch.cat([node_memory_hour.unsqueeze(-1),node_memory_day.unsqueeze(-1),subgraph_memory_hour,subgraph_memory_day],-1).reshape(node_memory_hour.shape[0],817,12,-1)))
